...
async def dbinit():
    async with aiosqlite.connect(r'E:\Git\Python\Project\connect.db') as db:
        try:
            await db.execute("CREATE TABLE user (username text primary key, password text not null)")
        except Exception as e:
            await db.execute("drop table user")
            await db.execute("CREATE TABLE user (username text primary key, password text not null)")
        await db.execute("INSERT INTO user (username,password) VALUES('lty', '2018211349')")
        await db.execute("INSERT INTO user (username,password) VALUES('zzl', '2018211346')")
        await db.execute("INSERT INTO user (username,password) VALUES('hzz', '2018211362')")
        await db.commit()
...
async def handle_echo(reader, writer):
    ...
    async with aiosqlite.connect(r'E:\Git\Python\Project\connect.db') as db:
        async with db.execute(f"SELECT password FROM user where username='{username}'") as cursor:
